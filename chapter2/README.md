...
概括的讲：2.2.4节和2.2.5节主要讲了在计算机中以及C语言中，有无符号转换的本质：保持位不变！！！！
2.2.6和2.2.7节，主要讲了类型放大时的位补齐和位缩小，这么说可能觉得难以理解。没关系，下面咱们通过举例，来一一揭晓二进制位的精华！
继续回到2.2.1节，我们看到，64位系统上，C语言的整型char,short,int,long等(long long先忽略)的取值范围；
那我们想在计算机中表示和存储某个数字，就比如说-1吧，应该如何表示呢？
按照有符号数中负数的表示原理（补码）以及-1本身数字本身的大小，我们发现，-1用一个字节就可以搞定，所以可以表示-128 + 127 ，即11111111
但是我们非要用2个字节表示可不可以呀？那当然可以，就是11111111 11111111 ，即-32768 + 32767 = -1 
那4个字节呢？就是-2147483648 + 2147483647 ，即11111111 11111111 11111111 11111111 
那8个字节呢？ 就是-9223372036854775808 + 9223372036854775807 ，
即11111111 11111111 11111111 11111111 11111111 11111111 11111111 11111111 
从这个例子我们看到，人类设计计算机语言时，就比如C语言，一般都是设计来多种不同长度到类型，比如char(1个字节，8位）
short(2个字节，16位),int(4个字节，32位），long（8个字节，64位），主要目的就是供咱们按需所取。
就拿刚才-1的例子来讲，我们可以用64的有符号long来存，也可以用8位的有符号char类存，这样一对比，优势就出来来，当然选char占用的内存更少呀。
更节省资源呀！！
我们继续来分析-1的例子：
当用char 类型存储-1时，它是8位，二进制是11111111(FF)，我们可以用show_byte程序来打印一下-1的位表示，
